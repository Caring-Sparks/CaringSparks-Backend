...
exports.bulkUpdateInfluencerStatus = exports.getInfluencerStats = exports.deleteInfluencer = exports.updateInfluencerStatus = exports.getInfluencerById = exports.getInfluencers = exports.updateInfluencer = exports.createInfluencer = void 0;
const cloudinary_1 = __importDefault(require("../utils/cloudinary"));
const bcrypt_1 = __importDefault(require("bcrypt"));
const crypto_1 = __importDefault(require("crypto"));
const Influencer_1 = __importDefault(require("../models/Influencer"));
const influencerEmailService_1 = require("../services/influencerEmailService");
const createInfluencer = async (req, res) => {
    try {
        ...
        const { name, email, phone, whatsapp, location, niches, audienceLocation, malePercentage, femalePercentage, } = parsedBody;
        ...
        const existingInfluencer = await Influencer_1.default.findOne({
        ...
        for (const platform of platforms) {
            ...
            if (platformData && typeof platformData === "object") {
                ...
                if (followers && url && impressions) {
                    ...
                    const proofFile = filesMap[`${platform}[proof]`] || // Frontend sends this format first
                        ...
                        const uploadPromise = new Promise((resolve, reject) => {
                            const uploadStream = cloudinary_1.default.uploader.upload_stream({
                                ...
                                if (error) {
                                    ...
                                    console.log(`[v0] ${platform} upload success:`, result?.secure_url);
        ...
        const audienceProofFile = filesMap.audienceProof ||
        ...
        const influencer = new Influencer_1.default(influencerData);
        await influencer.save();
        try {
            await (0, influencerEmailService_1.sendInfluencerWelcomeEmail)(influencer, plainPassword);
        ...
        try {
            await (0, influencerEmailService_1.sendAdminNotificationEmail)(influencer);
        }
...
const updateInfluencer = async (req, res) => {
    try {
        ...
        const existingInfluencer = await Influencer_1.default.findById(id);
        if (!existingInfluencer) {
            res.status(404).json({
                success: false,
        ...
        for (const platform of platforms) {
            ...
            if (platformData && typeof platformData === "object") {
                ...
                if (followers !== undefined &&
                    ...
                    const existingPlatformData = existingInfluencer[platform];
                    updates[platform] = {
                    ...
                    const proofFile = filesMap[`${platform}[proof]`] ||
                        filesMap[`${platform}.proof`] ||
                        filesMap[`${platform}Proof`] ||
                        filesMap[`${platform}_proof`] ||
                        filesMap[platform] ||
                        ...
                        const uploadPromise = new Promise((resolve, reject) => {
                            const uploadStream = cloudinary_1.default.uploader.upload_stream({
                                ...
                                if (error) {
                                    ...
                                    console.log(`${platform} upload success:`, result?.secure_url);
                                    ...
                                    if (existingPlatformData?.proofUrl) {
                                        try {
                                            ...
                                            console.error(`Failed to delete old ${platform} proof file:`, deleteError);
                                        }
                                    }
                    ...
                    if (error) {
                        ...
                        if (existingInfluencer.audienceProofUrl) {
                            try {
                                ...
                                console.error("Failed to delete old audience proof file:", deleteError);
                            }
                        }
        ...
        if (uploadPromises.length > 0) {
            try {
                console.log(`Starting ${uploadPromises.length} file uploads for update`);
                await Promise.all(uploadPromises);
        ...
        const updatedInfluencer = await Influencer_1.default.findByIdAndUpdate(id, updates, {
            new: true,
            runValidators: true,
        }).select("-password");
...
const getInfluencers = async (req, res) => {
    try {
        ...
        const totalCount = await Influencer_1.default.countDocuments(filter);
        const influencers = await Influencer_1.default.find(filter)
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit)
            .select("-password");
...
const getInfluencerById = async (req, res) => {
    try {
        ...
        const influencer = await Influencer_1.default.findById(id).select("-password");
        if (!influencer) {
            res.status(404).json({
                success: false,
                error: "Influencer not found",
...
const updateInfluencerStatus = async (req, res) => {
    try {
        ...
        const influencer = await Influencer_1.default.findByIdAndUpdate(id, { status }, { new: true }).select("-password");
        if (!influencer) {
            res.status(404).json({
                success: false,
                error: "Influencer not found",
...
exports.updateInfluencerStatus = updateInfluencerStatus;
// Delete influencer
const deleteInfluencer = async (req, res) => {
    try {
        ...
        const influencer = await Influencer_1.default.findById(id);
        if (!influencer) {
            res.status(404).json({
                success: false,
                error: "Influencer not found",
        ...
        platforms.forEach((platform) => {
            const platformData = influencer[platform];
        ...
        await Influencer_1.default.findByIdAndDelete(id);
        res.status(200).json({
            success: true,
...
const getInfluencerStats = async (req, res) => {
    try {
        const totalInfluencers = await Influencer_1.default.countDocuments();
        const pendingInfluencers = await Influencer_1.default.countDocuments({
            status: "pending",
        });
        const approvedInfluencers = await Influencer_1.default.countDocuments({
            status: "approved",
        });
        const rejectedInfluencers = await Influencer_1.default.countDocuments({
            status: "rejected",
        });
        // Recent registrations (last 30 days)
        const recentInfluencers = await Influencer_1.default.countDocuments({
            createdAt: { $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) },
        });
        // Platform distribution
        const platformStats = await Influencer_1.default.aggregate([
            {
                $project: {
                    platforms: {
                        $filter: {
        ...
        const locationStats = await Influencer_1.default.aggregate([
            { $group: { _id: "$location", count: { $sum: 1 } } },
            { $sort: { count: -1 } },
            { $limit: 10 },
        ]);
        // Top niches
        const nicheStats = await Influencer_1.default.aggregate([
            { $unwind: "$niches" },
        ...
        const earningsStats = await Influencer_1.default.aggregate([
            {
                $match: {
                    status: "approved",
...
const bulkUpdateInfluencerStatus = async (req, res) => {
    try {
        ...
        const invalidIds = influencerIds.filter((id) => !id.match(/^[0-9a-fA-F]{24}$/));
        ...
        const result = await Influencer_1.default.updateMany({ _id: { $in: influencerIds } }, { status, updatedAt: new Date() });
        res.status(200).json({
            success: true,
...
exports.bulkUpdateInfluencerStatus = bulkUpdateInfluencerStatus;
