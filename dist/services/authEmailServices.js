...
exports.testEmailConfiguration = exports.sendPasswordResetConfirmationEmail = exports.sendPasswordResetEmail = void 0;
...
const getPasswordResetEmailTemplate = (email, resetToken, role) => {
...
const getPasswordResetConfirmationTemplate = (email) => {
...
const sendPasswordResetEmail = async (email, resetToken, role) => {
    try {
        ...
        if (!email || !resetToken || !role) {
            throw new Error("Missing required parameters: email, resetToken, and role are required");
        }
...
const sendPasswordResetConfirmationEmail = async (email) => {
    try {
        ...
        const result = await transporter.sendMail(mailOptions);
        console.log(`Password reset confirmation email sent successfully to ${email}:`, result.messageId);
    }
    catch (error) {
