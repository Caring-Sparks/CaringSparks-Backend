...
export const sendPaymentConfirmationEmail = async (
  ...
  const campaignTitle = campaign.title || `${campaign.role || 'Campaign'} for ${brandName}`;
  const campaignBudget = campaign.totalCost || campaign.totalBaseCost || campaign.budget || 0;
...
<body>
  <div class="email-container">
    ...
    <div class="content">
      ...
      <div class="campaign-details">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">${campaign.platforms?.join(', ') || 'N/A'}</span>
        </div>
        <div class="detail-row">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">#${campaign._id.toString().slice(-8).toUpperCase()}</span>
        </div>
      </div>
    ...
    <div class="footer">
      <p class="footer-text">
        This email was sent regarding your ${campaign.role || 'campaign'} on CaringSparks.
      </p>
  ...
  try {
    ...
    console.log(`Payment confirmation email sent successfully to ${brandName} (${to})`);
  } catch (error) {
...
export const sendInfluencersAssignedEmail = async (
  ...
  const influencersList = assignedInfluencers.map((influencer, index) => `
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px; background-color: ${index % 2 === 0 ? '#f8fafc' : '#ffffff'}; border-radius: 6px; margin-bottom: 8px;">
      <div>
        <div style="font-weight: 600; color: #374151;">${influencer.name}</div>
        <div style="font-size: 13px; color: #64748b;">${influencer.email}</div>
        ${influencer.location ? `<div style="font-size: 12px; color: #9ca3af;">📍 ${influencer.location}</div>` : ''}
      </div>
...
<body>
  <div class="email-container">
    ...
    <div class="content">
      <p class="intro-text">
        Excellent news, <strong>${brandName}</strong>! We've successfully assigned ${assignedInfluencers.length} qualified influencers to your "${campaignTitle}" campaign.
      </p>
      
      <div class="status-card">
        <h2 class="status-title">👥 Team Assembled</h2>
        <div class="status-message">
          Your campaign now has a dedicated team of ${assignedInfluencers.length} influencer${assignedInfluencers.length > 1 ? 's' : ''} ready to create amazing content for your brand!
        </div>
      </div>
      ...
      <div class="campaign-summary">
        ...
        <div class="summary-row">
          ...
          <span class="summary-value">${campaign.platforms?.join(', ') || 'N/A'}</span>
        </div>
        <div class="summary-row">
        ...
        <div class="summary-row">
          ...
          <span class="summary-value">#${campaign._id.toString().slice(-8).toUpperCase()}</span>
        </div>
      </div>
  ...
  try {
    await transporter.sendMail(mailOptions);
    console.log(`Influencers assigned email sent successfully to ${brandName} (${to})`);
  } catch (error) {
